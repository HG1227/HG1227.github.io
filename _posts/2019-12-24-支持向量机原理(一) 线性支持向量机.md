...
在分离超平面固定为 $$w^Tx + b = 0$$ 的时候，$\| w^Tx + b \| $ 表示点x到超平面的相对距离。通过观察 $w^Tx + b$ 和 y 是否同号，我们判断分类是否正确，这些知识我们在感知机模型里都有讲到。这里我们引入函数间隔的概念，定义**函数间隔** $\gamma^{'}$ 为：
...
假设我们有S个支持向量，则对应我们求出S个 $b^{*}$ ,理论上这些 $b^{*}$ 都可以作为最终的结果， 但是我们一般采用一种更健壮的办法，即求出所有支持向量所对应的 $b_s^{*}$ ，然后将其平均值作为最后的结果。注意到对于严格线性可分的 SVM，$b$ 的值是有唯一解的，也就是这里求出的所有b∗b∗都是一样的，这里我们仍然这么写是为了和后面加入软间隔后的 SVM 的算法描述一致。
...
1. <a hre="https://www.cnblogs.com/pinard/p/6097604.html" target='_blank'> 支持向量机原理(一) 线性支持向量机</a> 